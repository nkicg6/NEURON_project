...
def ap_calculate_amplitude(y: np.ndarray, ind: int, rmp: float) -> float:
    ...
    assert len(y) > ind, f"Index out of range. length array = {len(y)}, index = {ind}"
    assert rmp < 0, f"RMP should be negative, did you make a mistake? RMP = {rmp}"
    amp_peak = y[ind]
...
def ap_calculate_fwhm(x: np.ndarray, first_ind: int, second_ind: int) -> float:
    ...
    ), f"Second ind ({second_ind}) must be > first ind ({first_ind})."
    return float(x[second_ind] - x[first_ind])


def ap_batch_fwhm(list_of_times: list, list_of_traces: list, rmp: float) -> list:
    assert len(list_of_times) == len(
        list_of_traces
    ), f"Length of time ({len(list_of_times)}) != to length of traces ({len(list_of_traces)})."
    fwhms = []
